...
TYPES t_output TYPE string.TYPES tt_output
TYPE STANDARD TABLE OF t_output WITH NON-UNIQUE DEFAULT KEY.
...
*             / lines( passenger_flights ) .
     ...
     r_result = REDUCE #(
                 INIT i = 0
                  FOR <flight> IN passenger_flights
                  WHERE ( table_line IS INSTANCE OF lcl_passenger_flight ) NEXT i +=
                  CAST lcl_passenger_flight( <flight> )->get_free_seats( )
             / pf_count ) .
